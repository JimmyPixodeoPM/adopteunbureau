...
<?php if ($has_items): ?>
	...
	<p class="woocommerce-mini-cart__total total">
		<?php if (has_action('woocommerce_widget_shopping_cart_total')): ?>
			...
			?>
		<?php else: ?>
			<strong class="text-v-dark text-uppercase"><?php esc_html_e('Total', 'woocommerce'); ?>:</strong>
	...
	foreach (WC()->session->get('shipping_for_package_0')['rates'] as $method_id => $rate) {
		if (WC()->session->get('chosen_shipping_methods')[0] == $method_id) {
			...
			echo '<p class="shipping-total">
				<strong class="label-shipping">' . $rate_label . ': </strong>
				<span class="text-price-shipping">' . WC()->cart->get_cart_shipping_total() . '</span>
			</p>';
			break;
		}
	}
