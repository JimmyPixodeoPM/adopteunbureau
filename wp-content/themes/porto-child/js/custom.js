    ...
    var wpmc_checkout = {
        ...
        init: function () {
            ...
            $('#wpmc-next-step, #wpmc-skip-login').on('click', function () {
                    ...
                    if (!$this.val()) {
                        ...
                        if ($this.attr('type') === 'email') {
                            ...
                            if (!emailRegex.test($this.val())) {
                                ...
                                if ($errorSpan.length === 0) {
                                    $errorSpan = $('<span class="error-message"></span>').text(emailErrorMessage);
                                    $this.after($errorSpan);
                                }
                                $this.addClass('error');
                            } else {
                ...
                if (isValid) {
                    wpmc_checkout.switch_tab(wpmc_checkout.current_index() + 1);
                }

            });

            // Click on "previous" button
            $('#wpmc-prev-step').on('click', function () {
        ...
        switch_tab: function (theIndex) {
            ...
            $('html, body').promise().done(function () {
                ...
                if (theIndex < self.$sections.length - 1) {
                    $('#wpmc-next-step').addClass('current');
                }
                ...
                if (theIndex === 0 && $('.wpmc-step-login').length > 0) {
                    ...
                    $('#wpmc-next-step').removeClass('current');
                    $('.woocommerce-NoticeGroup-checkout:not(wpmc-error)').hide();
                }

                // Last section
                if (theIndex === self.$sections.length - 1) {
                    $('#wpmc-prev-step').addClass('current');
        ...
        customizations: function () {
            if (WPMC.hide_last_prev === '1') {
                $('.woocommerce-checkout').on('wpmc_after_switching_tab', function () {
                    $('#wpmc-prev-step').css('visibility', $('.wpmc-tabs-wrapper li.wpmc-tab-item:last-child').hasClass('current') ? 'hidden' : 'visible');
                });
            }
