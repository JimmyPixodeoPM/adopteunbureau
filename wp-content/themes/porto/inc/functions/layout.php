...
function porto_currency_switcher( $el_class = '' ) {
	...
	if ( $porto_settings['wcml-switcher'] && '' == $porto_settings['wcml-switcher-pos'] ) {
		if ( class_exists( 'WOOCS' ) ) {
			...
			foreach ( $currencies as $currency ) {
				if ( $current_language && ( ! isset( $woocommerce_wpml->settings['currency_options'][ $currency ]['languages'][ $current_language ] ) || 1 != $woocommerce_wpml->settings['currency_options'][ $currency ]['languages'][ $current_language ] ) ) {
					continue;
				}
				if ( $currency && $woocommerce_wpml->multi_currency ) {
...
function porto_search_form( $el_class = '' ) {
	...
	if ( isset( $porto_settings['search-layout'] ) ) {
		$el_class  = $el_class ? ( $el_class . ' ' ) : $el_class;
		$el_class .= $porto_settings['search-layout'] . '-search-layout';
		if ( 'advanced' == $porto_settings['search-layout'] ) {
			if ( ! empty( $porto_settings['show-searchform-mobile'] ) ) {
				$el_class .= ' ssm-advanced-search-layout';
			}
		} elseif ( 'simple' == $porto_settings['search-layout'] || 'large' == $porto_settings['search-layout'] ) {
			$el_class .= ' search-dropdown';
		}
	}
	if ( $porto_settings['search-border-radius'] ) {
		$el_class .= ' search-rounded';
	}
...
function porto_header_socials( $el_class = '' ) {
	...
	if ( $porto_settings['header-social-twitter'] ) :
		?>
		<a target="_blank" <?php echo porto_filter_output( $nofollow ); ?> class="share-twitter" href="<?php echo esc_url( $porto_settings['header-social-twitter'] ); ?>" title="<?php esc_attr_e( 'X', 'porto' ); ?>"></a>
		<?php
	endif;
	...
	if ( ! empty( $porto_settings['header-social-spotify'] ) ) :
		?>
		<a target="_blank" <?php echo porto_filter_output( $nofollow ); ?> class="share-spotify" href="<?php echo esc_url( $porto_settings['header-social-spotify'] ); ?>" title="<?php esc_attr_e( 'Spotify', 'porto' ); ?>"></a>
		<?php
	endif;
	...
	if ( ! isset( $porto_header_builder_layout ) ) {
		...
		if ( 'header_builder' == $porto_settings['header-type-select'] ) {
			...
			if ( is_customize_preview() && ! empty( $selected_layout ) ) {
				...
				$porto_header_builder_layout['name'] = isset( $selected_layout['selected_layout'] ) ? $selected_layout['selected_layout'] : '';
			...
			if ( $builder_id ) {
				$builder_hd_type = get_post_meta( $builder_id, 'header_type', true );
				if ( ! metadata_exists( 'post', $builder_id, 'header_type' ) ) {
					$builder_hd_type = -1;
				}
				$porto_header_builder_layout = array(
					'ID'                => $builder_id,
					'type'              => $builder_hd_type,
					'side_header_width' => intval( get_post_meta( $builder_id, 'header_side_width', true ) ),
					'side_header_pos'   => get_post_meta( $builder_id, 'header_side_pos', true ),
					// 'is_fixed'          => 'absolute' == $builder_hd_type,
				);
			}
		}
	}
...
if ( ! function_exists( 'porto_woocommerce_product_nav' ) ) :
	function porto_woocommerce_product_nav() {
		...
		if ( apply_filters( 'porto_legacy_mode', true ) && empty( $porto_settings['product-nav'] ) ) {
			return;
		}
		...
		if ( is_array( $element ) ) {
			...
			foreach ( $element as $key => $value ) {
				if ( 'porto_block' == $key && $value ) {
					...
					$the_id  = get_the_ID();
					$hd_type = get_post_meta( $the_id, 'header_type', true );
					if ( porto_header_type_is_side() ) {
						echo porto_header_side_menu();
					} else if ( 'side' == $hd_type && 'porto_builder' == get_post_type( $the_id ) ) {
						echo porto_header_side_menu();
					} else {
