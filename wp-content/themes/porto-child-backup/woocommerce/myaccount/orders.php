...
<h3 class="account-sub-title bbb d-none d-md-block mb-3 mt-2"><i class="Simple-Line-Icons-social-dropbox align-middle m-r-sm"></i><?php esc_html_e( 'Orders', 'woocommerce' ); ?></h3>
								...
								if ( version_compare( WC_VERSION, '3.2', '>=' ) ) {
									...
									if ( ! empty( $actions ) ) {
										foreach ( $actions as $key => $action ) { // phpcs:ignore WordPress.WP.GlobalVariablesOverride.OverrideProhibited
											...
											echo '<a href="' . esc_url( $action['url'] ) . '" class="woocommerce-button button wc-action-btn px-4 ' . sanitize_html_class( $key ) . '" aria-label="' . esc_attr( sprintf( __( 'View order number %s', 'woocommerce' ), $order->get_order_number() ) ) . '">' . esc_html( $action['name'] ) . '</a>';
										}
									}
								} else {
									$actions = array(
