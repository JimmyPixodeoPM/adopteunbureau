	...
	class WC_CPM_Admin_Settings {
		...
		public function enqueue_admin_css_js() {
			...
			$localized_strings = array(
				...
				'selectProductTaxonomy'               => __( 'Select product taxonomy&hellip;', 'conditional-payment-methods-for-woocommerce' ),
			);
			...
			$js_params = array(
				'security'         => wp_create_nonce( CPM_AJAX_SECURITY ),
		...
		public function output() {
			...
			if ( 'conditional_payment_methods_for_woocommerce' === $current_section ) {
				wp_nonce_field( 'wc_cpm_settings', CPM_AJAX_SECURITY, false );
				?>
				<p>
		...
		public function save_admin_settings() {
			...
			$security = ( ! empty( $_POST[ CPM_AJAX_SECURITY ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ CPM_AJAX_SECURITY ] ) ) : '';

			if ( empty( $security ) || ! wp_verify_nonce( $security, 'wc_cpm_settings' ) ) { // phpcs:ignore
				return;
			}
