...
if ( ! class_exists( 'WC_CPM_Controller' ) ) {
	...
	class WC_CPM_Controller {
		...
		public function filter_payment_gateways( $payment_gateways ) {
			...
			if ( empty( $payment_method_conditions ) || empty( $payment_method_conditions['enabled'] ) ) {
			...
			foreach ( $payment_method_conditions['enabled'] as $enabled_conditions ) {
				...
				if ( empty( $enabled_conditions['rules'] ) ) {
					continue;
				}

				foreach ( $enabled_conditions['rules'] as $rule ) {
					switch ( $rule['field'] ) {
						case 'cart_subtotal':
							$obj = new WC_CPM_By_Cart();
							break;

						case ( false !== strpos( $rule['field'], 'taxonomy_' ) ):
