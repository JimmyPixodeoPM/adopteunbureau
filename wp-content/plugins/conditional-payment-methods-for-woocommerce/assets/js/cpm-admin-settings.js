		...
		var cpmScripts = function() {
			try {
				...
				$( '#wc_cpm_data' ).find( '.cpm-select2-taxonomy' ).select2({
					...
					ajax: {
						dataType : 'json',
						url : cpmSettingsParams.ajaxURL,
						data: function (params) {
							return {
								taxonomy: (select2Element) ? $(select2Element).data('taxonomy') : '',
								search: params.term,
								page: params.page || 1,
								action : 'cpm_get_taxonomy_list',
								security: cpmSettingsParams.security
							}
						},
						// paging
						processResults : function(response) {
							return {
								results : response.data || [],// Background returned datasets
								pagination : {
									more: response.load_more || false// Total number of pages is 10, so 1-9 pages can be refreshed drop-down
								}
							};
						},
						cache : false,
