...
if ( ! empty( $product ) && isset( $atts['link_source'] ) ) {
	...
	if ( 'cart' == $atts['link_source'] ) {
		...
		if ( apply_filters( 'porto_product_loop_show_price', true ) ) {
			...
			if ( ! empty( $atts['show_quantity_input'] ) && $product->is_type( 'simple' ) && $product->is_purchasable() && $product->is_in_stock() ) {
			...
			echo apply_filters(
				...
				sprintf(
					'<%s %s title="%s" href="%s"%s data-quantity="%s" class="' . esc_attr( apply_filters( 'porto_elements_wrap_css_class', $btn_classes, $atts, 'porto-tb/porto-woo-buttons' ) ) . ' %s" %s%s>%s</%s>',
					$tag,
					$simple_cart_html,
