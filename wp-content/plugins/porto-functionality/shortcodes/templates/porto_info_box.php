...
extract(
	shortcode_atts(
		array(
			...
			'enable_field_title_dynamic'                => false,
			'field_title_dynamic_source'                => '',
			'field_title_dynamic_content'               => '',
			'field_title_dynamic_content_meta_field'    => '',
			'field_title_dynamic_before'                => '',
			'field_title_dynamic_after'                 => '',
			'field_title_dynamic_fallback'              => '',
			'field_title_date_format'                   => '',
			
			'enable_field_subtitle_dynamic'             => false,
			'field_subtitle_dynamic_source'             => '',
			'field_subtitle_dynamic_content'            => '',
			'field_subtitle_dynamic_content_meta_field' => '',
			'field_subtitle_dynamic_before'             => '',
			'field_subtitle_dynamic_after'              => '',
			'field_subtitle_dynamic_fallback'           => '',
			'field_subtitle_date_format'                => '',
			'date_format'                               => ''
		),
		$atts,
		'porto_info_box'
...
if ( $mobile_pos ) {
	$classes  .= ' porto-sicon-mobile';
	if ( is_array( $mobile_spacing ) && ( '' !== $mobile_spacing['left'] || '' !== $mobile_spacing['right'] ||'' !== $mobile_spacing['top'] ||'' !== $mobile_spacing['bottom'] ) ) {
		$classes  .= ' porto-sicon-mspace';
	}
}
if ( $inf_design_style ) {
...
if ( '' !== $title_margin_top && is_string( $title_margin_top ) ) {
	$unit = trim( preg_replace( '/[0-9.-]/', '', $title_margin_top ) );
	if ( ! $unit ) {
		$title_margin_top .= 'px';
	}
	$title_style .= 'margin-top:' . esc_attr( $title_margin_top ) . ';';
}
...
if ( $enable_field_title_dynamic ) {
	if ( ( 'meta_field' == $field_title_dynamic_source ) && ! empty( $field_title_dynamic_content_meta_field ) ) {
		$title = Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $field_title_dynamic_source, $field_title_dynamic_content_meta_field, 'field' );
	}
	if ( ! empty( $field_title_dynamic_content ) ) {
		if ( '' == $field_title_date_format && '' !== $date_format ) {
			// Backward Compatibility
			$field_title_date_format = $date_format;
		}
		if ( ! empty( $field_title_date_format ) ) {
			$field_title_dynamic_content = array(
				'field_title_dynamic_content' => $field_title_dynamic_content,
				'field_title_date_format'     => $field_title_date_format,
			);
		}
		$title = Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $field_title_dynamic_source, $field_title_dynamic_content, 'field', 'title' );
	}
	if ( empty( $title ) ) {
		$title = $field_title_dynamic_fallback;
	}

	$title = $field_title_dynamic_before . $title . $field_title_dynamic_after;
}

if ( $enable_field_subtitle_dynamic ) {
	if ( ( 'meta_field' == $field_subtitle_dynamic_source ) && ! empty( $field_subtitle_dynamic_content_meta_field ) ) {
		$subtitle = Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $field_subtitle_dynamic_source, $field_subtitle_dynamic_content_meta_field, 'field' );
	}
	if ( ! empty( $field_subtitle_dynamic_content ) ) {
		if ( ! empty( $field_subtitle_date_format ) ) {
			$field_subtitle_dynamic_content = array(
				'field_subtitle_dynamic_content' => $field_subtitle_dynamic_content,
				'field_subtitle_date_format'     => $field_subtitle_date_format,
			);
		}
		$subtitle = Porto_Func_Dynamic_Tags_Content::get_instance()->dynamic_get_data( $field_subtitle_dynamic_source, $field_subtitle_dynamic_content, 'field', 'subtitle' );
	}
	if ( empty( $subtitle ) ) {
		$subtitle = $field_subtitle_dynamic_fallback;
	}

	$subtitle = $field_subtitle_dynamic_before . $subtitle . $field_subtitle_dynamic_after;
}
