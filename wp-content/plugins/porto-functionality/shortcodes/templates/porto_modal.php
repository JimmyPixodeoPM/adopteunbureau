...
if ( 'youtube' == $modal_contain || 'vimeo' == $modal_contain ) {
	$trigger_id   = isset( $atts[ $modal_contain . '_url' ] ) ? $atts[ $modal_contain . '_url' ] : '';
	$content_type = 'iframe';
} else {
	$uniq = uniqid( '', true );
...
if ( 'image' == $modal_on && $btn_img ) {
	$alt_text = '';
	if ( is_numeric( $btn_img ) ) {
		$img      = wp_get_attachment_image_src( $btn_img, 'full' );
		$alt_text = get_post_meta( $btn_img, '_wp_attachment_image_alt', true );
	} else if ( is_array( $btn_img ) && ! empty( $btn_img['id'] ) ) {
		$img = wp_get_attachment_image_src( $btn_img['id'], 'full' );
		$alt_text = get_post_meta( $btn_img['id'], '_wp_attachment_image_alt', true );
	} else if ( is_array( $btn_img ) && ! empty( $btn_img['url'] ) ) {
		$img = $btn_img['url'];
		if ( ! empty( $btn_img['alt'] ) ) {
			$alt_text = $btn_img['alt'];
		}
	}
	...
	if ( isset( $img ) && is_array( $img ) ) {
		...
		$html    .= '<img src="' . esc_url( $img ) . '" alt="' . esc_attr( $alt_text ) . '" data-trigger-id="' . esc_attr( $trigger_id ) . '" data-type="' . $content_type . '" class="porto-modal-trigger img-responsive" data-overlay-class="' . esc_attr( $modal_style ) . '" />';
	}
	...
	( function() {
		...
		if ( window.jQuery && jQuery("' . esc_js( $modal_on_selector ) . '").length ) {
			porto_init_modal();
		} else {
