...
class MONETICO_Update_API_Check {
	...
	public function update_check( $transient ) {
		...
		// Récupération de la date d’expiration de la licence transmise en bonus lors de l’appel de l’API
		// Nicolas Maillard
		if(is_object($response)):
			$access_expires = (string)$response->access_expires;
			set_transient( 'access_expires_monetico', date('Y-m-d', strtotime($access_expires)), 3600*24 );
		endif;
