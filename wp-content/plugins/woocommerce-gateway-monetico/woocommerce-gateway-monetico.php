 ...
 * @category    Plugin
 * @copyright   Copyright (c) 2000-2023, Nicolas Maillard ABSOLUTE Web
...
function init_gateway_monetico() {
	...
	if(!defined("PASSERELLE_MONETICO_VERSION")) define("PASSERELLE_MONETICO_VERSION", "2.4.5");
	...
	class WC_Gateway_Monetico extends WC_Payment_Gateway {
		...
		public function generate_monetico_form( $order_id ) {
			...
			if(!empty($fractionne_monetico)):
				...
				$pourcents = apply_filters( 'monetico_change_pourcents_fractionnements', $pourcents, $order);
				...
				for($i=2;$i<=$echeances;$i++):
				  ...
				  $ech[$i-1]["montant"] = number_format($total_cde*$pourcents[$echeances.'x'][$i-2]/100, 2, ".", "");
					$cumul_montants += $ech[$i-1]["montant"];
				  $ech[$i]["date"] = abw_new_anniv( date("Y-m-d"), $i-1 );
				endfor;
				$ech[$echeances]["montant"] = number_format($total_cde-$cumul_montants, 2, ".", "");
			endif;
			...
			$libelleMonetique = substr(apply_filters( 'monetico_change_libelle_monetique', '', $order ), 0, 32);
	...
	\"shipping\":{
		...
		\"matchBillingAddress\":".$matchBillingAddress."
			...
			?>
				<iframe id="iframePaiement" name="iframePaiement" width="100%" height="330" frameborder="0" src="<?php echo $oEpt->sUrlPaiement; ?>?version=<?php echo $oEpt->sVersion;?>&TPE=<?php echo $oEpt->sNumero;?>&date=<?php echo urlencode($sDate);?>&contexte_commande=<?php echo $sContexteCommande;?>&ThreeDSecureChallenge=<?php echo $ThreeDSecureChallenge;?>&montant=<?php echo $sMontant.$sDevise;?>&reference=<?php echo $sReference;?>&MAC=<?php echo $sMAC;?>&url_retour_ok=<?php echo urlencode($oEpt->sUrlOK);?>&url_retour_err=<?php echo urlencode($oEpt->sUrlKO);?>&lgue=<?php echo $oEpt->sLangue;?>&libelleMonetique=<?php echo $libelleMonetique;?>&societe=<?php echo $oEpt->sCodeSociete;?>&texte-libre=<?php echo HtmlEncode($sTexteLibre);?>&mail=<?php echo urlencode($sEmail);?>&protocole=<?php echo $protocole;?>&desactivemoyenpaiement=<?php echo $desactivemoyenpaiement;?>&aliascb=<?php echo $aliascb;?>&mode_affichage=iframe&nbrech=<?php echo $fractionne_monetico;?><?php echo $fraction; ?>"></iframe>
			<?php
			else:
              ...
              <p>
                  ...
                  <input type="hidden" name="libelleMonetique"      id="libelleMonetique"       value="<?php echo $libelleMonetique;?>" />
  ...
  <?php		
              if($monetico_settings['redirection']=='yes'&&$monetico_settings['debug']!='yes'){
                  ...
                  wc_enqueue_js( 'setTimeout(function(){jQuery("#bouton").click();}, '.apply_filters( 'monetico_change_delai_redirection', 0).')' );
              }
			endif; // Iframe or not
			if($monetico_settings['debug']=='yes'){
				...
				echo "<strong>libelleMonetique :</strong> ".$libelleMonetique."<br/>";
